...
namespace RendezvousWrestling.FightSystem.Fight
{
    public class RWFighterState : BaseFighterState<RWAchievement, RWAchievementManager, RWActionFactory, RWActionType, RWActiveAction, RWDataContext, RWEntityMapper, RWFeature, RWFeatureFactory, RWFeatureParameter, RWFeatureType, RWFight, RWFighterState, RWFighterStats, RendezVousWrestlingGame, RWModifier, RWModifierFactory, RWModifierParameters, RWModifierType, RWUser>
    {
        ...
        public override string OutputStatus()
        {
            ...
            var livesLine = $"  [color=red]lives: {RemainingLivesBarDisplay}{(OrgasmsDamageLastRound > 0 || OrgasmsHealLastRound > 0 ? $"{ (-OrgasmsDamageLastRound + OrgasmsHealLastRound < 0 ? "[color=red]" : "[color=green]")} ({RendezvousWrestling.Common.Utils.GlobalUtils.GetSignedNumber(-OrgasmsDamageLastRound + OrgasmsHealLastRound)} orgasm(s))[/color]" : "")}{ (HeartsDamageLastRound > 0 || HeartsHealLastRound > 0 ? $"{ (-HeartsDamageLastRound + HeartsHealLastRound < 0 ? "[color=red]" : "[color=green]")} ({ RendezvousWrestling.Common.Utils.GlobalUtils.GetSignedNumber(-HeartsDamageLastRound + HeartsHealLastRound)})[/color]" : "")} ({LivesRemaining}|{MaxLives})[/color] ";
