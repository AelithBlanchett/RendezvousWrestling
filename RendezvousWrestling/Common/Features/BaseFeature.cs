...
namespace RendezvousWrestling.Common.Features
{
        ...
        where TUser : BaseUser<TAchievement, TAchievementManager, TActionFactory, TActionType, TActiveAction, TDataContext, TEntityMapper, TFeature, TFeatureFactory, TFeatureParameters, TFeatureType, TFight, TFighterState, TFighterStats, TFightingGame, TModifier, TModifierFactory, TModifierParameters, TModifierType, TUser>, new()
    {
        ...
        public BaseFeature(TFeatureType featureType, int uses)
        {
            Initialize(featureType, uses);
        }

        public void Initialize(TFeatureType featureType, int uses)
        {
            Type = featureType;
            Uses = uses;
        }

        public void Activate(TUser receiver)
        {
            Receiver = receiver;
        }
        ...
        public string Trigger(TriggerMoment moment, TriggerEvent triggeringEvent, TFeatureParameters parameters)
        {
            var triggeredFeatureMessage = ApplyFeature(moment, triggeringEvent, parameters);
            ...
            {
                messageAboutFeature = $"{Receiver.Id} is affected by the {Type}, {triggeredFeatureMessage}";
            }
        ...
        public abstract string ApplyFeature(TriggerMoment moment, TriggerEvent triggeringEvent, TFeatureParameters parameters);
